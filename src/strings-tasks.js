 ...
 */
function concatenateStrings(exam1, exam2) {
  const str = exam1.concat(exam2);
  return str;
}
 ...
 *   getFirstChar('') => ''
 */
function getFirstChar(exam) {
  const rezult = exam.charAt(0);
  return rezult;
}
...
function formatTime(minutes, seconds) {
  const addMinutes = String(minutes).padStart(2, '0');
  const addSeconds = String(seconds).padStart(2, '0');
  return `${addMinutes}:${addSeconds}`;
}
...
function isPalindrome(str) {
  const casel = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  const reverseStr = casel.split('').reverse().join('');
  return casel === reverseStr;
}
...
function findLongestWord(sentence) {
  const box = sentence.split(' ');
  let rezult = '';
  for (let i = 0; i < box.length; i += 1) {
    const word = box[i];
    if (word.length > rezult.length) {
      rezult = box[i];
    }
  }
  return rezult;
